...
public class MController {
	...
	@RequestMapping(value = "/api/v1/moderators", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	@ExceptionHandler(Exception.class)
	@ResponseBody
	public ResponseEntity<Moderator> addModerator(@RequestBody Moderator mod, Exception e, @RequestHeader(value = "Authorization") String auth) {
	...
	@RequestMapping(value = "api/v1/moderators/{moderator_id}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@ExceptionHandler(Exception.class)
	public ResponseEntity<Moderator> viewModerator(
			@PathVariable("moderator_id") Integer mod_id, Exception e, @RequestHeader(value = "Authorization") String auth) {
		 if (Authenticate.isValid(auth)) {
		if (mod_id == null)
		{
	...
	/* Update Moderator */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@RequestMapping(value = "api/v1/moderators/{moderator_id}", method = RequestMethod.PUT, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	@ExceptionHandler(Exception.class)
	@ResponseBody
	public ResponseEntity<Moderator> updateModerator(
		...
		else if (mod.getPassword() == null || mod.getPassword().isEmpty())
		{
			...
			return new ResponseEntity<Moderator>(updateMod, HttpStatus.CREATED);
		}
	}
    else{
